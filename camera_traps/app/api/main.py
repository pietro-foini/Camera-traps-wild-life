...
async def lifespan(app: FastAPI):
    ...
    if Path(S.CLASSIFIER_MODEL_PATH).suffix == ".keras":
        from camera_traps.app.models.tf_classifier import TFImageClassifier
        ...
        from camera_traps.app.models.tflite_classifier import TFLiteImageClassifier
    ...
    if Path(S.DETECTOR_MODEL_PATH).suffix == ".pt":
        from camera_traps.app.models.pt_detector import PTImageDetector

        detector = PTImageDetector()
        detector.load(S.DETECTOR_MODEL_PATH)

    elif Path(S.DETECTOR_MODEL_PATH).suffix == ".tflite":
        from camera_traps.app.models.tflite_detector import TFLiteImageDetector
